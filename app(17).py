    ...
    if "selected" not in st.session_state: st.info("Calculate a feature ranking and select features first.")
        ...
        if st.button("Train and evaluate models", type="primary", disabled=not selected_models):
            ...
            if val.empty or test.empty: st.error("Validation or test data is empty. Each pH-temperature group needs at least 3 curves.")
                ...
                # A separate regression model estimates temperature directly from CV-derived features.
                # Temperature is intentionally excluded from its inputs to prevent target leakage.
                temperature_model=make_pipeline(
                    SimpleImputer(strategy="median"),
                    HistGradientBoostingRegressor(max_iter=220,learning_rate=.06,l2_regularization=.2,random_state=RANDOM_STATE),
                )
                temperature_model.fit(xtr,train.temperature_C)
                temperature_validation=np.asarray(temperature_model.predict(xv)).reshape(-1)
                temperature_test=np.asarray(temperature_model.predict(xt)).reshape(-1)
                temperature_metrics={
                    "Validation_R2":r2_score(val.temperature_C,temperature_validation),
                    "Validation_RMSE":np.sqrt(mean_squared_error(val.temperature_C,temperature_validation)),
                    "Test_R2":r2_score(test.temperature_C,temperature_test),
                    "Test_RMSE":np.sqrt(mean_squared_error(test.temperature_C,temperature_test)),
                    "Test_MAE":mean_absolute_error(test.temperature_C,temperature_test),
                }
                st.session_state.update(results=results, models=fitted, predictions=predictions, test_frame=test,
                                        temperature_model=temperature_model,temperature_predictions=temperature_test,
        ...
        if "results" in st.session_state:
            ...
            if "temperature_metrics" in st.session_state:
                tm=st.session_state.temperature_metrics; t1,t2,t3=st.columns(3)
                t1.metric("Temperature-model test R²",f"{tm['Test_R2']:.3f}")
                t2.metric("Temperature-model test RMSE",f"{tm['Test_RMSE']:.2f} °C")
                t3.metric("Temperature-model test MAE",f"{tm['Test_MAE']:.2f} °C")
...
with prediction_tab:
    if "results" not in st.session_state:
        ...
        predicted_temperature = (float(st.session_state.temperature_predictions[random_index])
                                 if "temperature_predictions" in st.session_state else np.nan)
        ...
        p3.metric("🌡️ Actual temperature", f"{float(sample['temperature_C']):g} °C")
        p4.metric("🌡️ Predicted temperature", "Retrain model" if not np.isfinite(predicted_temperature) else f"{predicted_temperature:.2f} °C")
        p5.metric("📏 pH absolute error", f"{absolute_error:.3f}")
        ...
        report = pd.DataFrame({
            ...
            "absolute_error": [absolute_error], "actual_temperature_C": [sample["temperature_C"]],
            "predicted_temperature_C": [predicted_temperature],
        })
        st.download_button(
...
with live_test_tab:
    ...
    st.caption("Import a two-column potential/current CSV from MIT App Inventor. The app extracts CV features and independently predicts temperature and pH.")
    if "results" not in st.session_state or "models" not in st.session_state:
        ...
        if live_file is not None:
            try:
                ...
                st.info("Temperature is not read from the CSV. A separately trained CV-feature model estimates it automatically.")
                u1,u2=st.columns(2)
                ...
                if st.button("🧠 Extract features and predict live pH",type="primary",use_container_width=True):
                    ...
                    if "temperature_model" not in st.session_state: raise ValueError("Retrain the models once to create the new temperature-prediction model.")
                    raw=pd.Series({"sample_id":"MIT_Live_Test","pH":np.nan,"temperature_C":np.nan,"replicate":1,"potential_V":voltage,"current_A":current*factor,"source_file":live_file.name})
                    ...
                    temperature=float(np.asarray(st.session_state.temperature_model.predict(model_input)).reshape(-1)[0])
                    raw["temperature_C"]=temperature; feature_values["temperature_C"]=temperature
        ...
        if "live_result" in st.session_state:
            ...
            m2.metric("🌡️ Predicted temperature",f"{live['temperature']:.2f} °C")
            ...
            summary=pd.DataFrame({"source_file":[live["filename"]],"location":[location_name],"date_time":[local_now.strftime("%Y-%m-%d %H:%M:%S %Z")],"pH_model":[live["model"]],"predicted_temperature_C":[live["temperature"]],"actual_pH":[actual],"predicted_pH":[predicted],"absolute_error":[None if actual is None else abs(predicted-actual)]})
