...
def rank_features(train_features):
    ...
    ranking = pd.DataFrame({"Feature": clean.columns, "F_Score": fscore, "P_Value": pvalue,
                            ...
                            "Pearson_r_with_pH": pearson_r.reindex(clean.columns).to_numpy(),
...
def feature_selection_study(ranking, selected_features):
    """Create a readable, evidence-based explanation for selected features."""
    selected = ranking[ranking["Feature"].isin(selected_features)].copy()
    method_columns = {
        "ANOVA F-test": "F_Score",
        "mutual information": "Mutual_Information",
        "Random Forest": "RF_Importance",
        "permutation importance": "Permutation_Importance",
    }
    top_cutoff = max(1, int(np.ceil(len(ranking) * .25)))
    method_ranks = {
        label: ranking[column].rank(method="min", ascending=False)
        for label, column in method_columns.items()
    }
    rows = []
    for index, row in selected.iterrows():
        r = float(row["Pearson_r_with_pH"])
        magnitude = abs(r)
        strength = (
            "very strong" if magnitude >= .8 else
            "strong" if magnitude >= .6 else
            "moderate" if magnitude >= .4 else
            "weak" if magnitude >= .2 else "very weak"
        )
        direction = "positive" if r > 0 else "negative" if r < 0 else "no linear"
        supporting_methods = [
            label for label, ranks in method_ranks.items() if ranks.loc[index] <= top_cutoff
        ]
        support_text = ", ".join(supporting_methods) if supporting_methods else "combined evidence across methods"
        significant = bool(row["Statistically_Significant"])
        reason = (
            f"Ranked #{int(row['Rank'])} by the combined score; supported by {support_text}. "
            f"It has a {strength} {direction} linear relationship with pH (r={r:.3f}). "
            + ("The ANOVA association remains significant after multiple-testing correction."
               if significant else
               "The ANOVA association is not significant after multiple-testing correction, so interpret it cautiously.")
        )
        rows.append({
            "Feature": row["Feature"],
            "Combined rank": int(row["Rank"]),
            "Combined score": row["Combined_Score"],
            "Pearson r with pH": r,
            "Linear relationship": f"{strength}, {direction}",
            "Adjusted p-value": row["Adjusted_P_Value"],
            "Supporting methods (top 25%)": support_text,
            "Why selected": reason,
        })
    return pd.DataFrame(rows).sort_values("Combined rank").reset_index(drop=True)
...
def dual_prediction_gif(raw,predicted_ph,actual_ph,predicted_temperature,actual_temperature,ph_model,temperature_model):
    """Animate one CV acquisition with independent pH and temperature predictions."""
    voltage=np.asarray(raw["potential_V"],float); current=np.asarray(raw["current_A"],float)*1e9; frames=[]
    for fraction in np.linspace(.08,1,20):
        end=max(3,int(len(voltage)*fraction)); evolving_ph=(actual_ph+predicted_ph)/2+(predicted_ph-(actual_ph+predicted_ph)/2)*fraction; evolving_temp=(actual_temperature+predicted_temperature)/2+(predicted_temperature-(actual_temperature+predicted_temperature)/2)*fraction
        fig,(ax1,ax2,ax3)=plt.subplots(1,3,figsize=(12,3.8),gridspec_kw={"width_ratios":[1.8,1,1]})
        ax1.plot(voltage[:end],current[:end],color="#6f2dbd",lw=2.2); ax1.fill_between(voltage[:end],current[:end],color="#00a6c7",alpha=.13); ax1.set_xlim(voltage.min(),voltage.max()); pad=max(np.ptp(current)*.08,.1); ax1.set_ylim(current.min()-pad,current.max()+pad); ax1.set(xlabel="Potential (V)",ylabel="Current (nA)",title=f"CV acquisition · {fraction:.0%}"); ax1.grid(alpha=.2)
        ax2.scatter(.5,evolving_ph,s=650,c="#d33b83",edgecolor="white",lw=3); ax2.text(.5,evolving_ph,f"{evolving_ph:.2f}",ha="center",va="center",color="white",weight="bold"); ax2.axhline(actual_ph,color="#00856a",ls="--"); ax2.set(xlim=(0,1),ylim=(min(actual_ph,predicted_ph)-1,max(actual_ph,predicted_ph)+1),ylabel="pH",title=f"pH · {ph_model}"); ax2.set_xticks([])
        ax3.scatter(.5,evolving_temp,s=650,c="#f59e0b",edgecolor="white",lw=3); ax3.text(.5,evolving_temp,f"{evolving_temp:.1f}°",ha="center",va="center",color="white",weight="bold"); ax3.axhline(actual_temperature,color="#00856a",ls="--"); tpad=max(abs(predicted_temperature-actual_temperature)+5,8); ax3.set(xlim=(0,1),ylim=(min(actual_temperature,predicted_temperature)-tpad,max(actual_temperature,predicted_temperature)+tpad),ylabel="Temperature (°C)",title=f"Temperature · {temperature_model}"); ax3.set_xticks([])
        fig.suptitle("Dual AI prediction from one CV · dashed line = experimental reference",weight="bold"); fig.tight_layout(); buffer=io.BytesIO(); fig.savefig(buffer,format="png",dpi=85,bbox_inches="tight"); plt.close(fig); buffer.seek(0); frames.append(Image.open(buffer).convert("P"))
    output=io.BytesIO(); frames[0].save(output,format="GIF",save_all=True,append_images=frames[1:],duration=160,loop=0); return output.getvalue()
...
with st.sidebar:
    st.markdown(
        """
        <div style="margin:.15rem 0 1.15rem;padding:1rem .85rem;border-radius:16px;
                    background:linear-gradient(135deg,rgba(0,166,199,.34),rgba(111,66,193,.46));
                    border:1px solid rgba(255,255,255,.30);box-shadow:0 8px 22px rgba(0,0,0,.16);
                    text-align:center;line-height:1.5">
          <div style="font-size:1.06rem;font-weight:800;letter-spacing:.01em">⚗️ AI-TC pH Sensor App</div>
          <div style="font-size:.72rem;opacity:.80;letter-spacing:.12em;margin:.15rem 0 .45rem">RESEARCH APPLICATION</div>
          <div style="font-size:.94rem;font-weight:700">Developed by Akshaya AV</div>
          <div style="font-size:.72rem;opacity:.72">© 2026</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
with upload_tab:
    ...
    if st.button("Read uploaded CV data", type="primary", disabled=not uploads):
        try:
            ...
            for key in ["features", "ranking", "selected", "results", "models", "predictions", "temperature_results", "temperature_models", "temperature_predictions"]: st.session_state.pop(key, None)
...
with feature_tab:
    if "split_data" not in st.session_state: st.info("Upload and read your Excel files first.")
        ...
        with st.spinner("Extracting electrochemical and signal-shape features..."):
            ...
            for key in ["ranking", "selected", "results", "models", "predictions", "temperature_results", "temperature_models", "temperature_predictions"]: st.session_state.pop(key, None)
...
with selection_tab:
    st.markdown("### Feature selection — simple explanation")
    st.markdown(
        """
        Feature selection asks one simple question: **which CV measurements contain the most useful information about pH?**

        1. **F-test (`f_regression`) — “Does this feature change with pH in a straight-line pattern?”**  
           It compares the useful linear change with the unexplained variation. A high F-score and a small adjusted p-value mean stronger evidence that the feature is linearly related to pH. It is often called an ANOVA F-test, but here pH is treated as a continuous number.

        2. **Mutual information — “Does knowing this feature help us guess pH?”**  
           It finds both straight-line and curved relationships. A larger value means the feature contains more information about pH.

        3. **Random Forest importance — “How much does this feature help the trees make better decisions?”**  
           A forest of decision trees predicts pH. Features that create larger improvements when the trees split the data receive higher importance.

        4. **Permutation importance — “What happens if we scramble this feature?”**  
           One feature is randomly mixed up. If prediction becomes much worse, that feature was important. This is a practical check of usefulness.

        5. **Combined score — “What is the overall evidence?”**  
           The four scores above are put on the same 0–1 scale and averaged. The highest combined scores are selected.

        **Pearson correlation is an interpretation tool.** `r` ranges from −1 to +1: positive means the feature rises as pH rises, negative means it falls, and a value near zero means no clear straight-line change. Pearson `r` is shown separately and is not added to the combined score.
        """
    )
    ...
    if "ranking" in st.session_state:
        ...
        feature_codes = {
            feature: f"F{number}"
            for number, feature in enumerate(st.session_state.selected, start=1)
        }
        feature_key = pd.DataFrame({
            "Code": list(feature_codes.values()),
            "Full feature name": [name.replace("_", " ") for name in feature_codes],
        })
        selection_study = feature_selection_study(ranking, st.session_state.selected)
        ...
        train_target = st.session_state.features.query("dataset == 'Training'").pH.reset_index(drop=True)
        pearson_with_ph = (
            st.session_state.clean_train[st.session_state.selected]
            .corrwith(train_target)
            .to_frame(name="pH")
            .T
        ).rename(columns=feature_codes)
        heatmap_width = max(9, min(18, 0.65 * len(st.session_state.selected)))
        fig, ax = plt.subplots(figsize=(heatmap_width, 3.2))
        sns.heatmap(
            pearson_with_ph,
            annot=True,
            fmt=".3f",
        ...
        ax.set_xlabel("Selected CV feature code")
        ax.set_ylabel("Target")
        ax.set_title("Pearson correlation: target pH vs selected features")
        ax.tick_params(axis="x", rotation=45)
        fig.tight_layout()
        st.pyplot(fig, use_container_width=True)
        plt.close(fig)
        st.markdown("### How selected features change with pH and temperature")
        st.caption(
            "These are descriptive heatmaps. The first shows relative feature values; the next two show "
            "Pearson correlations. Use all curves to study the experiment, or training curves only to keep "
            "the analysis completely separate from validation and testing."
        )
        heatmap_scope = st.radio(
            "Data used in pH–temperature heatmaps",
            ["All curves", "Training curves only"],
            horizontal=True,
            key="feature_heatmap_scope",
        )
        heatmap_data = st.session_state.features.copy()
        if heatmap_scope == "Training curves only":
            heatmap_data = heatmap_data.query("dataset == 'Training'").copy()
        heatmap_data = heatmap_data[
            ["pH", "temperature_C"] + st.session_state.selected
        ].replace([np.inf, -np.inf], np.nan)

        st.markdown("#### 1. Feature level at every pH and temperature")
        st.caption(
            "Each cell is a standardized mean (z-score): red = higher than that feature's overall mean, "
            "blue = lower, and white = close to the mean. Standardization lets features with different units be compared."
        )
        feature_profile = heatmap_data.groupby(
            ["pH", "temperature_C"], as_index=True
        )[st.session_state.selected].mean()
        profile_sd = feature_profile.std(axis=0, ddof=0).replace(0, np.nan)
        standardized_profile = (
            (feature_profile - feature_profile.mean(axis=0)) / profile_sd
        ).fillna(0).rename(columns=feature_codes)
        standardized_profile.index = [
            f"pH {ph:g} | {temperature:g} °C"
            for ph, temperature in standardized_profile.index
        ]
        fig, ax = plt.subplots(figsize=(
            max(10, min(18, .65 * len(st.session_state.selected))),
            max(6, min(24, .62 * len(standardized_profile) + 2)),
        ))
        sns.heatmap(
            standardized_profile,
            cmap="coolwarm",
            center=0,
            annot=len(standardized_profile) * len(st.session_state.selected) <= 180,
            fmt=".2f",
            cbar_kws={"label": "Standardized mean feature value (z-score)"},
            ax=ax,
        )
        ax.set(xlabel="Selected CV feature code", ylabel="Experimental condition",
               title="Relative feature change across pH and temperature")
        ax.tick_params(axis="x", rotation=0)
        ax.tick_params(axis="y", rotation=0)
        fig.subplots_adjust(left=.16, right=.94, bottom=.13, top=.91)
        st.pyplot(fig, use_container_width=True); plt.close(fig)

        st.markdown("#### 2. Feature relationship with pH at each temperature")
        st.caption(
            "Read across one row to see which features rise (+) or fall (−) with pH while temperature is held fixed."
        )
        ph_correlation_rows = {}
        for temperature, group in heatmap_data.groupby("temperature_C"):
            if group["pH"].nunique() >= 2:
                ph_correlation_rows[f"{temperature:g} °C"] = group[
                    st.session_state.selected
                ].corrwith(group["pH"])
        ph_by_temperature = pd.DataFrame.from_dict(
            ph_correlation_rows, orient="index"
        ).rename(columns=feature_codes)
        if ph_by_temperature.empty:
            st.warning("At least two different pH values are required at the same temperature for this heatmap.")
        else:
            fig, ax = plt.subplots(figsize=(max(10, min(18, .65 * len(st.session_state.selected))),
                                             max(6, .72 * len(ph_by_temperature) + 2)))
            sns.heatmap(ph_by_temperature, cmap="coolwarm", center=0, vmin=-1, vmax=1,
                        annot=True, fmt=".2f", cbar_kws={"label": "Pearson r with pH"}, ax=ax)
            ax.set(xlabel="Selected CV feature code", ylabel="Temperature",
                   title="Feature–pH correlation calculated separately at each temperature")
            ax.tick_params(axis="x", rotation=0)
            ax.tick_params(axis="y", rotation=0)
            fig.subplots_adjust(left=.12, right=.94, bottom=.13, top=.89)
            st.pyplot(fig, use_container_width=True); plt.close(fig)

        st.markdown("#### 3. Feature relationship with temperature at each pH")
        st.caption(
            "Read across one row to see which features rise (+) or fall (−) with temperature while pH is held fixed."
        )
        temperature_correlation_rows = {}
        for ph_value, group in heatmap_data.groupby("pH"):
            if group["temperature_C"].nunique() >= 2:
                temperature_correlation_rows[f"pH {ph_value:g}"] = group[
                    st.session_state.selected
                ].corrwith(group["temperature_C"])
        temperature_by_ph = pd.DataFrame.from_dict(
            temperature_correlation_rows, orient="index"
        ).rename(columns=feature_codes)
        if temperature_by_ph.empty:
            st.warning("At least two different temperatures are required at the same pH for this heatmap.")
        else:
            fig, ax = plt.subplots(figsize=(max(10, min(18, .65 * len(st.session_state.selected))),
                                             max(5.5, .72 * len(temperature_by_ph) + 2)))
            sns.heatmap(temperature_by_ph, cmap="coolwarm", center=0, vmin=-1, vmax=1,
                        annot=True, fmt=".2f", cbar_kws={"label": "Pearson r with temperature"}, ax=ax)
            ax.set(xlabel="Selected CV feature code", ylabel="pH",
                   title="Feature–temperature correlation calculated separately at each pH")
            ax.tick_params(axis="x", rotation=0)
            ax.tick_params(axis="y", rotation=0)
            fig.subplots_adjust(left=.10, right=.94, bottom=.14, top=.88)
            st.pyplot(fig, use_container_width=True); plt.close(fig)

        st.markdown("#### Feature-code key")
        st.dataframe(feature_key, use_container_width=True, hide_index=True, height=420)

        st.markdown("### Why these features were selected")
        st.write(
            "The selected variables are the highest-ranked features after combining four complementary "
            "selection methods. Agreement across methods is stronger evidence than relying on one statistic alone."
        )
        st.dataframe(
            selection_study.style.format({
                "Combined score": "{:.3f}",
                "Pearson r with pH": "{:.3f}",
                "Adjusted p-value": "{:.3g}",
            }),
            use_container_width=True,
            hide_index=True,
        )
        with st.expander("Feature-by-feature interpretation", expanded=True):
            for _, study_row in selection_study.iterrows():
                st.markdown(f"- **{study_row['Feature']}** — {study_row['Why selected']}")
        st.info(
            "Interpretation note: selection identifies variables that are useful for predicting pH in this "
            "training dataset; it does not prove that pH directly causes the feature to change. Temperature, "
            "electrode batch, scan conditions, and correlated features may also influence the ranking. "
            "Final evidence should come from validation and test performance on independent CV curves."
        )
        ...
        st.download_button("Download feature ranking", excel_bytes({"Feature_Ranking": ranking, "Selected_Features": pd.DataFrame({"Feature":st.session_state.selected}), "Selection_Study": selection_study}), "CV_feature_selection.xlsx")
    ...
    if "selected" not in st.session_state: st.info("Calculate a feature ranking and select features first.")
        ...
        if st.button("Train and evaluate models", type="primary", disabled=not selected_models):
            ...
            if val.empty or test.empty: st.error("Validation or test data is empty. Each pH-temperature group needs at least 3 curves.")
                ...
                temperature_rows=[]; temperature_fitted={}; temperature_predictions={}
                ...
                for idx,name in enumerate(selected_models):
                    ...
                    progress.progress((idx+1)/(2*len(selected_models)))
                for idx,name in enumerate(selected_models):
                    try:
                        temperature_model=clone(models[name]).fit(xtr,train.temperature_C)
                        tv=np.asarray(temperature_model.predict(xv)).reshape(-1); tt=np.asarray(temperature_model.predict(xt)).reshape(-1)
                        temperature_fitted[name]=temperature_model; temperature_predictions[name]={"validation":tv,"test":tt}
                        temperature_rows.append({"Model":name,"Validation_R2":r2_score(val.temperature_C,tv),"Validation_RMSE":np.sqrt(mean_squared_error(val.temperature_C,tv)),"Validation_MAE":mean_absolute_error(val.temperature_C,tv),
                                                 "Test_R2":r2_score(test.temperature_C,tt),"Test_RMSE":np.sqrt(mean_squared_error(test.temperature_C,tt)),"Test_MAE":mean_absolute_error(test.temperature_C,tt)})
                    except Exception as exc: st.warning(f"Temperature {name} skipped: {exc}")
                    progress.progress((len(selected_models)+idx+1)/(2*len(selected_models)))
                ...
                temperature_results=pd.DataFrame(temperature_rows).sort_values(["Validation_RMSE","Validation_MAE"]).reset_index(drop=True); temperature_results.insert(0,"Rank",range(1,len(temperature_results)+1))
                st.session_state.update(results=results, models=fitted, predictions=predictions, test_frame=test,
                                        temperature_results=temperature_results,temperature_models=temperature_fitted,
                                        temperature_predictions=temperature_predictions)
        if "results" in st.session_state:
            result_ph,result_temp=st.tabs(["🧪 pH training results","🌡️ Temperature training results"])
            with result_ph:
                st.success(f"Best pH model: {st.session_state.results.iloc[0].Model}")
                st.dataframe(st.session_state.results.style.format(precision=4), use_container_width=True)
            with result_temp:
                if "temperature_results" in st.session_state:
                    st.success(f"Best temperature model: {st.session_state.temperature_results.iloc[0].Model}")
                    st.dataframe(st.session_state.temperature_results.style.format(precision=4), use_container_width=True)
                else: st.info("Click Train and evaluate models again to create temperature models.")

def render_target_comparison(target_label,results,predictions,models,test,actual,classes,color_values,color_label,unit,key_prefix):
    metric=st.selectbox("Comparison metric",["Validation_RMSE","Validation_MAE","Validation_R2","Test_RMSE","Test_MAE","Test_R2"],key=f"{key_prefix}_metric")
    fig,ax=plt.subplots(figsize=(9,5)); ordered=results.sort_values(metric,ascending="R2" not in metric); sns.barplot(data=ordered,x=metric,y="Model",ax=ax,palette="viridis"); ax.set_title(f"{target_label} model comparison",fontweight="bold"); fig.tight_layout(); st.pyplot(fig); plt.close(fig)
    name=st.selectbox("Detailed model plots",results.Model,key=f"{key_prefix}_model"); predicted=np.asarray(predictions[name]["test"]).reshape(-1)
    c1,c2=st.columns(2)
    with c1:
        fig,ax=plt.subplots(figsize=(6.2,5.7)); points=ax.scatter(actual,predicted,c=color_values,cmap="plasma",s=72,edgecolor="white",linewidth=.7,label=f"Predicted {target_label}")
        lo=min(actual.min(),predicted.min()); hi=max(actual.max(),predicted.max()); ideal,=ax.plot([lo,hi],[lo,hi],"k--",lw=2,label="Ideal prediction")
        ax.set_xlabel(f"Actual {target_label}",fontweight="bold"); ax.set_ylabel(f"Model-predicted {target_label}",fontweight="bold"); ax.set_title(f"{name}\nActual vs Predicted {target_label}",fontweight="bold",pad=12); ax.grid(alpha=.18); fig.colorbar(points,ax=ax,pad=.02,label=color_label); ax.legend(handles=[points,ideal],loc="upper center",bbox_to_anchor=(.5,-.16),frameon=False,fontsize=8); fig.subplots_adjust(bottom=.25); st.pyplot(fig); plt.close(fig)
    with c2:
        fig,ax=plt.subplots(figsize=(5.7,5.2)); actual_classes=nearest(actual,classes); plotted=False
        for target_class in classes:
            binary=(actual_classes==target_class).astype(int)
            if binary.min()==binary.max(): continue
            score=-np.abs(predicted-target_class); fpr,tpr,_=roc_curve(binary,score); ax.plot(fpr,tpr,lw=2,label=f"{target_class:g}{unit} (AUC={auc(fpr,tpr):.2f})"); plotted=True
        ax.plot([0,1],[0,1],"k--",alpha=.6); ax.set(xlabel="False-positive rate",ylabel="True-positive rate",title=f"{name}: One-vs-Rest ROC")
        if plotted: ax.legend(fontsize=7,loc="upper center",bbox_to_anchor=(.5,-.15),ncol=2,frameon=False)
    ...
    m1,m2,m3=st.columns(3); m1.metric("Test R²",f"{plot_r2:.4f}"); m2.metric("Test RMSE",f"{plot_rmse:.3f} {unit}"); m3.metric("Test MAE",f"{plot_mae:.3f} {unit}")
    classified=nearest(predicted,classes); cm=confusion_matrix(nearest(actual,classes),classified,labels=classes); acc=accuracy_score(nearest(actual,classes),classified); bal=balanced_accuracy_score(nearest(actual,classes),classified)
    st.write(f"Nearest-class accuracy: **{acc:.1%}** · Balanced accuracy: **{bal:.1%}**")
    d1,d2=st.columns([1,1.25])
    with d1:
        fig,ax=plt.subplots(figsize=(6.2,5.3)); sns.heatmap(cm,annot=True,fmt="d",cmap="Blues",xticklabels=[f"{x:g}" for x in classes],yticklabels=[f"{x:g}" for x in classes],ax=ax); ax.set(xlabel=f"Predicted {target_label} class",ylabel=f"Actual {target_label} class",title=f"{name}: confusion matrix"); fig.tight_layout(); st.pyplot(fig); plt.close(fig)
    class_rows=[]
    for index,target_class in enumerate(classes):
        tp=cm[index,index]; fn=cm[index,:].sum()-tp; fp=cm[:,index].sum()-tp; tn=cm.sum()-tp-fn-fp
        precision=tp/(tp+fp) if tp+fp else 0; recall=tp/(tp+fn) if tp+fn else 0; specificity=tn/(tn+fp) if tn+fp else 0; f1=2*precision*recall/(precision+recall) if precision+recall else 0
        class_rows.append({"Class":target_class,"TP":tp,"TN":tn,"FP":fp,"FN":fn,"Precision":precision,"Recall":recall,"Specificity":specificity,"F1":f1})
    class_metrics=pd.DataFrame(class_rows)
    with d2: st.dataframe(class_metrics.style.format({"Precision":"{:.3f}","Recall":"{:.3f}","Specificity":"{:.3f}","F1":"{:.3f}"}),use_container_width=True,height=370)
    table=pd.DataFrame({"Sample ID":test.sample_id,f"Actual {target_label}":actual,f"Predicted {target_label}":predicted,"Absolute error":abs(predicted-actual)})
    st.dataframe(table,use_container_width=True)
    st.download_button(f"Download {target_label} model results",excel_bytes({"Model_Comparison":results,"Test_Predictions":table,"Class_Metrics":class_metrics}),f"CV_{key_prefix}_model_results.xlsx",key=f"{key_prefix}_results_download")
    buffer=io.BytesIO(); joblib.dump({"model":models[name],"selected_features":st.session_state.selected,"target":target_label,"classes":classes},buffer)
    st.download_button(f"Download trained {target_label} · {name}",buffer.getvalue(),f"{key_prefix}_{name.replace(' ','_')}_model.joblib",key=f"{key_prefix}_model_download")


with comparison_tab:
    if "results" not in st.session_state: st.info("Train the models first.")
    else:
        ph_compare,temp_compare=st.tabs(["🧪 pH model comparison","🌡️ Temperature model comparison"]); test=st.session_state.test_frame.reset_index(drop=True)
        with ph_compare:
            render_target_comparison("pH",st.session_state.results,st.session_state.predictions,st.session_state.models,test,test.pH.to_numpy(float),np.sort(st.session_state.features.pH.unique()),test.temperature_C.to_numpy(float),"Experimental temperature (°C)","pH","ph")
        with temp_compare:
            if "temperature_results" not in st.session_state: st.info("Retrain models to generate the temperature comparison.")
            else:
                render_target_comparison("Temperature",st.session_state.temperature_results,st.session_state.temperature_predictions,st.session_state.temperature_models,test,test.temperature_C.to_numpy(float),np.sort(st.session_state.features.temperature_C.unique()),test.pH.to_numpy(float),"Experimental pH","°C","temperature")
...
with prediction_tab:
    if "results" not in st.session_state:
        ...
        temperature_results = st.session_state.get("temperature_results")
        temperature_predictions = st.session_state.get("temperature_predictions",{})
        best_temperature_model = temperature_results.iloc[0]["Model"] if temperature_results is not None else None
        st.subheader("🎯 Random test-sample prediction")
        st.caption("A held-out CV predicts pH and temperature independently. Experimental temperature is hidden from both model inputs and used only after prediction for validation.")

        control_1, control_2, control_3 = st.columns([1.4,1.4,1])
        with control_1:
            prediction_model = st.selectbox(
                "pH prediction model", results["Model"].tolist(),
                index=results["Model"].tolist().index(best_model), key="random_prediction_model"
            )
        with control_2:
            if best_temperature_model is None: st.warning("Retrain models to enable temperature prediction."); temperature_prediction_model=None
            else: temperature_prediction_model=st.selectbox("Temperature prediction model",temperature_results["Model"].tolist(),index=temperature_results["Model"].tolist().index(best_temperature_model),key="random_temperature_model")
        with control_3:
        ...
        actual_temperature=float(sample["temperature_C"])
        predicted_temperature=(float(temperature_predictions[temperature_prediction_model]["test"][random_index]) if temperature_prediction_model else np.nan)
        temperature_error=abs(predicted_temperature-actual_temperature) if np.isfinite(predicted_temperature) else np.nan
        ...
        p3.metric("📏 pH error", f"{absolute_error:.3f}")
        p4.metric("🌡️ Predicted temperature","Retrain" if not np.isfinite(predicted_temperature) else f"{predicted_temperature:.2f} °C")
        p5.metric("Actual temperature",f"{actual_temperature:g} °C")
        p6.metric("Temperature error","—" if not np.isfinite(temperature_error) else f"{temperature_error:.2f} °C")
        ...
        if temperature_prediction_model:
            st.markdown("#### Temperature prediction from the same CV features")
            temperature_values=pd.DataFrame({"Model":["Actual temperature"]+temperature_results["Model"].tolist(),"Temperature_C":[actual_temperature]+[float(temperature_predictions[m]["test"][random_index]) for m in temperature_results["Model"]],"Type":["Experimental"]+["AI prediction"]*len(temperature_results)})
            fig,ax=plt.subplots(figsize=(10,5)); colors=["#047857"]+["#f59e0b"]*len(temperature_results); bars=ax.barh(temperature_values.Model,temperature_values.Temperature_C,color=colors)
            offset=max(np.ptp(temperature_values.Temperature_C)*.015,.35)
            for bar,value in zip(bars,temperature_values.Temperature_C): ax.text(bar.get_width()+offset,bar.get_y()+bar.get_height()/2,f"{value:.1f} °C",va="center",fontsize=8)
            ax.axvline(actual_temperature,color="#047857",ls="--",lw=1.6,label="Actual experimental temperature"); ax.set_xlabel("Temperature (°C)"); ax.set_title("Independent temperature prediction · test temperature not supplied to models",fontweight="bold"); ax.invert_yaxis(); ax.legend(loc="lower right"); fig.tight_layout(); st.pyplot(fig); plt.close(fig)
        ...
        if st.button("▶ Play dual live prediction stream",use_container_width=True,disabled=raw is None or temperature_prediction_model is None):
            ...
            for frame_number,end in enumerate(frame_ends,1):
                ...
                evolving_temperature=(actual_temperature+predicted_temperature)/2+(predicted_temperature-(actual_temperature+predicted_temperature)/2)*completion if np.isfinite(predicted_temperature) else np.nan
                ...
                live_chart.pyplot(fig,clear_figure=True,use_container_width=True); plt.close(fig); live_status.markdown(f"<div class='section-card'><span class='status-dot'></span><b>Streaming</b> · points {end}/{len(voltage)} · evolving <b>pH {evolving:.3f}</b> · <b>{evolving_temperature:.1f} °C</b></div>",unsafe_allow_html=True); live_progress.progress(frame_number/len(frame_ends)); time.sleep(.07)
            live_status.success(f"Acquisition complete · predicted pH {predicted_ph:.3f} · predicted temperature {predicted_temperature:.2f} °C")

        st.markdown("### 🎬 Downloadable prediction animation")
        gif_key=f"{sample['sample_id']}::{prediction_model}::{temperature_prediction_model}"
        if st.button("Generate dual pH + temperature animation",use_container_width=True,disabled=raw is None or temperature_prediction_model is None):
            with st.spinner("Animating CV acquisition and both evolving predictions..."):
                st.session_state.prediction_video=dual_prediction_gif(raw,predicted_ph,actual_ph,predicted_temperature,actual_temperature,prediction_model,temperature_prediction_model); st.session_state.prediction_video_key=gif_key
        if st.session_state.get("prediction_video_key")==gif_key:
            st.image(st.session_state.prediction_video,caption="Independent machine-learning pH and temperature prediction",use_container_width=True)
            st.download_button("Download dual prediction GIF",st.session_state.prediction_video,"AI_pH_temperature_prediction.gif",mime="image/gif",use_container_width=True)

        report = pd.DataFrame({
            "sample_id": [sample["sample_id"]], "location": [location_name],
            "date_time": [local_now.strftime("%Y-%m-%d %H:%M:%S %Z")],
            "latitude": [latitude], "longitude": [longitude], "pH_model": [prediction_model],"temperature_model":[temperature_prediction_model],
            "actual_pH": [actual_ph], "predicted_pH": [predicted_ph],
            "pH_absolute_error": [absolute_error], "actual_temperature_C": [actual_temperature],
            "predicted_temperature_C":[predicted_temperature],"temperature_absolute_error_C":[temperature_error],
        })
        st.download_button(
