...
def prediction_gif(raw,predicted_ph,actual_ph):
    ...
    start=(actual_ph+predicted_ph)/2
    for fraction in np.linspace(.08,1,18):
        ...
        ax2.scatter(.5,evolving,s=600,c="#d33b83",edgecolor="white",lw=3); ax2.axhline(actual_ph,color="#00856a",ls="--",label=f"Actual {actual_ph:g}"); ax2.text(.5,evolving,f"{evolving:.2f}",ha="center",va="center",color="white",weight="bold")
        ax2.set_xlim(0,1); ax2.set_ylim(min(actual_ph,predicted_ph)-1,max(actual_ph,predicted_ph)+1); ax2.set_xticks([]); ax2.set_ylabel("pH"); ax2.set_title("Evolving ML prediction"); ax2.legend(fontsize=8)
