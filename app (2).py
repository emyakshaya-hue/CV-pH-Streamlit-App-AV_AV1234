...
def create_pdf_and_powerpoint(location, timestamp, results, test, predicted, model_name):
...
with comparison_tab:
    if "results" not in st.session_state: st.info("Train the models first.")
        ...
        with c2:
            ...
            if plotted: ax.legend(fontsize=8,loc="lower right")
...
with prediction_tab:
    if "results" not in st.session_state:
        ...
        report_key=f"{prediction_model}::{location_name}::{len(test)}"
        if st.button("Generate PDF and PowerPoint reports",type="primary",use_container_width=True):
            try:
                with st.spinner("Creating the PDF and PowerPoint..."):
                    st.session_state.dynamic_reports=create_pdf_and_powerpoint(
                        location_name,local_now.strftime("%Y-%m-%d %H:%M:%S %Z"),
                        results,test,predictions[prediction_model]["test"],prediction_model,
                    )
                    st.session_state.dynamic_report_key=report_key
            except ImportError:
                st.error("PowerPoint support is not installed. Upload the supplied requirements.txt to GitHub and reboot the app.")
            except Exception as exc:
                st.error(f"Report generation failed: {exc}")
        if st.session_state.get("dynamic_report_key")==report_key:
            pdf_report,powerpoint_report=st.session_state.dynamic_reports; report_col1,report_col2=st.columns(2)
            report_col1.download_button("Download PDF report",pdf_report,"AI_TC_pH_Report.pdf",mime="application/pdf",use_container_width=True)
            report_col2.download_button("Download PowerPoint",powerpoint_report,"AI_TC_pH_Report.pptx",mime="application/vnd.openxmlformats-officedocument.presentationml.presentation",use_container_width=True)
