...
with selection_tab:
    ...
    st.markdown(
        """
        1. **ANOVA F-test (`f_regression`)** — measures each feature's linear association with pH and provides a p-value.
        2. **Mutual information** — detects linear and non-linear dependency between each feature and pH.
        3. **Random Forest importance** — measures the decrease in prediction impurity contributed by each feature.
        4. **Permutation importance** — measures how much model performance decreases when a feature is shuffled.
        5. **Combined score** — the mean of the normalized F-score, mutual information, Random Forest importance, and permutation importance.

        **Pearson correlation (`r`)** is also reported to show the direction and strength of the linear relationship between every feature and the target pH. It is displayed separately and is not included in the combined score.
        """
    )
