...
with selection_tab:
    ...
    if "ranking" in st.session_state:
        ...
        if len(st.session_state.selected) >= 3 and len(st.session_state.clean_train) >= 3:
            selected = st.session_state.selected
            training = st.session_state.features.query("dataset == 'Training'").reset_index(drop=True)
            x = StandardScaler().fit_transform(st.session_state.clean_train[selected])
            pca_model = PCA(n_components=3)
            scores = pca_model.fit_transform(x)
            ph_values = training["pH"].to_numpy(dtype=float)
            temperatures = training["temperature_C"].to_numpy(dtype=float)
            # Invalidate the previous animation if the input measurements or selected features change.
            signature_data = pd.concat([training[["pH", "temperature_C"]],
                                        st.session_state.clean_train[selected].reset_index(drop=True)], axis=1)
            signature = hashlib.sha256(pd.util.hash_pandas_object(signature_data, index=True).values.tobytes()
                                       + repr(selected).encode()).hexdigest()
            if st.session_state.get("pca_animation_signature") != signature:
                st.session_state.pop("pca_animation_bytes", None)
                st.session_state.pop("pca_animation_mime", None)
            st.markdown("### Rotating 3D PCA of training CV measurements")
            st.caption("Each point is one training CV curve. Colour represents measured pH; marker represents temperature. PCA is fitted to standardised selected features.")
            if st.button("Generate rotating 3D PCA video", key="generate_pca_video"):
                with st.spinner("Rendering the rotating PCA animation..."):
                    fig = plt.figure(figsize=(8.5, 6.5))
                    ax = fig.add_subplot(111, projection="3d")
                    markers = ["o", "^", "s", "D", "P", "X", "v", "*"]
                    colour_scale = plt.Normalize(vmin=float(np.min(ph_values)), vmax=float(np.max(ph_values)))
                    for idx, temperature in enumerate(np.sort(np.unique(temperatures))):
                        mask = temperatures == temperature
                        ax.scatter(scores[mask, 0], scores[mask, 1], scores[mask, 2],
                                   c=ph_values[mask], cmap="viridis", norm=colour_scale,
                                   marker=markers[idx % len(markers)], s=50, alpha=.85,
                                   label=f"{temperature:g} °C")
                    for axis_idx, name in enumerate(["PC1", "PC2", "PC3"]):
                        label = f"{name} ({pca_model.explained_variance_ratio_[axis_idx]:.1%})"
                        [ax.set_xlabel, ax.set_ylabel, ax.set_zlabel][axis_idx](label)
                    fig.colorbar(plt.cm.ScalarMappable(norm=colour_scale, cmap="viridis"),
                                 ax=ax, shrink=.65, pad=.1, label="Measured pH")
                    ax.legend(title="Temperature", loc="upper left", fontsize=8)
                    ax.set_title("PCA of selected CV features (training data)")
                    def rotate(frame):
                        ax.view_init(elev=24, azim=frame * 4)
                        return (ax,)
                    animation = FuncAnimation(fig, rotate, frames=90, interval=60)
                    use_mp4 = shutil.which("ffmpeg") is not None
                    suffix = ".mp4" if use_mp4 else ".gif"
                    try:
                        with tempfile.NamedTemporaryFile(suffix=suffix) as output:
                            writer = FFMpegWriter(fps=20, bitrate=1600) if use_mp4 else PillowWriter(fps=15)
                            animation.save(output.name, writer=writer, dpi=100)
                            st.session_state.pca_animation_bytes = open(output.name, "rb").read()
                        st.session_state.pca_animation_mime = "video/mp4" if use_mp4 else "image/gif"
                        st.session_state.pca_animation_signature = signature
                    except Exception as exc:
                        st.error(f"Could not render PCA animation: {exc}")
                    finally:
                        plt.close(fig)
            if st.session_state.get("pca_animation_bytes") and st.session_state.get("pca_animation_signature") == signature:
                animation_bytes = st.session_state.pca_animation_bytes
                mime = st.session_state.pca_animation_mime
                if mime == "video/mp4": st.video(animation_bytes)
                else: st.image(animation_bytes, caption="Rotating 3D PCA (GIF fallback)")
                st.download_button("Download rotating 3D PCA animation", animation_bytes,
                                   "CV_training_PCA_3D.mp4" if mime == "video/mp4" else "CV_training_PCA_3D.gif",
                                   mime=mime, key="download_pca_animation")
        else:
            st.info("Select at least three features and use at least three training measurements for a 3D PCA video.")
