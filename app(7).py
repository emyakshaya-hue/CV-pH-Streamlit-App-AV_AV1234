...
def rank_features(train_features):
    ...
    forest = RandomForestRegressor(n_estimators=50, max_depth=10, random_state=RANDOM_STATE, n_jobs=1).fit(clean, y)
    pi = permutation_importance(forest, clean, y, n_repeats=3, random_state=RANDOM_STATE, n_jobs=1)
...
def model_set(feature_count, train_count):
    ...
    return {"Linear Regression": scaled(LinearRegression()), "Ridge Regression": scaled(Ridge(alpha=1)),
            ...
            "Random Forest": plain(RandomForestRegressor(n_estimators=50, max_depth=10, random_state=RANDOM_STATE, n_jobs=1)),
            "Extra Trees": plain(ExtraTreesRegressor(n_estimators=50, max_depth=10, random_state=RANDOM_STATE, n_jobs=1)),
            "Gradient Boosting": plain(GradientBoostingRegressor(n_estimators=75, learning_rate=.06, max_depth=2, random_state=RANDOM_STATE)),
            "Histogram Gradient Boosting": plain(HistGradientBoostingRegressor(max_iter=75, learning_rate=.06, random_state=RANDOM_STATE))}
...
def classification_metrics(actual, predicted, classes):
    actual_c, predicted_c = nearest(actual, classes), nearest(predicted, classes)
    cm = confusion_matrix(actual_c, predicted_c, labels=classes)
    total=cm.sum(); tp=np.diag(cm); fp=cm.sum(0)-tp; fn=cm.sum(1)-tp; tn=total-(tp+fp+fn)
    specificity=np.divide(tn,tn+fp,out=np.zeros_like(tn,dtype=float),where=(tn+fp)!=0)
    return {"TP":int(tp.sum()),"TN":int(tn.sum()),"FP":int(fp.sum()),"FN":int(fn.sum()),
            "Accuracy":accuracy_score(actual_c,predicted_c),"Balanced Accuracy":balanced_accuracy_score(actual_c,predicted_c),
            "Precision":precision_score(actual_c,predicted_c,labels=classes,average="macro",zero_division=0),
            "Recall":recall_score(actual_c,predicted_c,labels=classes,average="macro",zero_division=0),
            "Specificity":float(np.mean(specificity)),"F1":f1_score(actual_c,predicted_c,labels=classes,average="macro",zero_division=0)}


def make_prediction_gif(raw, predicted_ph, actual_ph):
    voltage=np.asarray(raw["potential_V"],float); current=np.asarray(raw["current_A"],float)*1e9
    frames=[]; start_ph=(actual_ph+predicted_ph)/2
    for fraction in np.linspace(.08,1,18):
        end=max(3,int(len(voltage)*fraction)); evolving=start_ph+(predicted_ph-start_ph)*fraction
        fig,(ax1,ax2)=plt.subplots(1,2,figsize=(9,3.6),gridspec_kw={"width_ratios":[1.7,1]})
        ax1.plot(voltage[:end],current[:end],color="#6f2dbd",lw=2.2); ax1.set_xlim(voltage.min(),voltage.max())
        pad=max(np.ptp(current)*.08,.1); ax1.set_ylim(current.min()-pad,current.max()+pad)
        ax1.set(xlabel="Potential (V)",ylabel="Current (nA)",title="Live CV acquisition"); ax1.grid(alpha=.2)
        ax2.scatter(.5,evolving,s=600,c="#e83e8c",edgecolor="white",lw=3); ax2.axhline(actual_ph,color="#00856a",ls="--",label=f"Actual {actual_ph:g}")
        ax2.text(.5,evolving,f"{evolving:.2f}",ha="center",va="center",color="white",weight="bold")
        ax2.set_xlim(0,1); ax2.set_ylim(min(actual_ph,predicted_ph)-1,max(actual_ph,predicted_ph)+1); ax2.set_xticks([]); ax2.set_ylabel("pH"); ax2.set_title("Evolving prediction"); ax2.legend(fontsize=8)
        fig.suptitle("AI-TC pH Sensor App",weight="bold"); fig.tight_layout(); image_buffer=io.BytesIO(); fig.savefig(image_buffer,format="png",dpi=85,bbox_inches="tight"); plt.close(fig)
        image_buffer.seek(0); frames.append(Image.open(image_buffer).convert("P"))
    output=io.BytesIO(); frames[0].save(output,format="GIF",save_all=True,append_images=frames[1:],duration=160,loop=0)
    return output.getvalue()


def figure_bytes(fig):
    output=io.BytesIO(); fig.savefig(output,format="png",dpi=170,bbox_inches="tight",facecolor="white"); plt.close(fig); return output.getvalue()


def make_report_bundle(location,timestamp,results,sample,predicted,model,raw,feature_table):
    images={}
    fig,ax=plt.subplots(figsize=(11,7)); ax.axis("off"); ax.text(.05,.86,"AI-TC pH Sensor App",fontsize=28,weight="bold",color="#153b66")
    ax.text(.05,.73,f"Location: {location}\nDate/time: {timestamp}",fontsize=14,linespacing=1.7)
    ax.text(.05,.42,f"Sample: {sample['sample_id']}\nModel: {model}\nActual pH: {sample['pH']:g}\nPredicted pH: {predicted:.3f}\nAbsolute error: {abs(predicted-float(sample['pH'])):.3f}",fontsize=15,linespacing=1.5,bbox=dict(boxstyle="round,pad=.7",facecolor="#eef7ff"))
    images["01_summary.png"]=figure_bytes(fig)
    fig,ax=plt.subplots(figsize=(9,5)); ordered=results.sort_values("Test_RMSE"); ax.barh(ordered.Model,ordered.Test_RMSE,color=plt.cm.viridis(np.linspace(.15,.9,len(ordered)))); ax.set(xlabel="Test RMSE",title="Model comparison"); fig.tight_layout(); images["02_models.png"]=figure_bytes(fig)
    fig,ax=plt.subplots(figsize=(9,5)); ax.plot(raw["potential_V"],np.asarray(raw["current_A"])*1e9,color="#6f2dbd",lw=2.3); ax.set(xlabel="Potential (V)",ylabel="Current (nA)",title="Random test CV"); ax.grid(alpha=.2); fig.tight_layout(); images["03_test_cv.png"]=figure_bytes(fig)
    pdf=io.BytesIO()
    with PdfPages(pdf) as pages:
        for content in images.values():
            image=Image.open(io.BytesIO(content)); fig,ax=plt.subplots(figsize=(11,7)); ax.imshow(image); ax.axis("off"); pages.savefig(fig,bbox_inches="tight"); plt.close(fig)
    ppt=io.BytesIO(); deck=Presentation(); deck.slide_width=Inches(13.333); deck.slide_height=Inches(7.5)
    for title,content in zip(["Analysis summary","Model comparison","Random test CV"],images.values()):
        slide=deck.slides.add_slide(deck.slide_layouts[6]); box=slide.shapes.add_textbox(Inches(.5),Inches(.15),Inches(12),Inches(.5)); p=box.text_frame.paragraphs[0]; p.text=title; p.font.size=Pt(24); p.font.bold=True
        slide.shapes.add_picture(io.BytesIO(content),Inches(.6),Inches(.8),width=Inches(12),height=Inches(6.2))
    deck.save(ppt); png_zip=io.BytesIO()
    with zipfile.ZipFile(png_zip,"w",zipfile.ZIP_DEFLATED) as archive:
        for filename,content in images.items(): archive.writestr(filename,content)
    return pdf.getvalue(),ppt.getvalue(),png_zip.getvalue()
...
with feature_tab:
    ...
    if "features" in st.session_state:
        ...
        sphere_data=features[["pH","temperature_C",view_feature,"dataset","sample_id"]].replace([np.inf,-np.inf],np.nan).dropna()
        st.success("Sphere-only feature visualization is active — the old boxplot has been removed.")
        st.markdown("#### Interactive 3D spherical feature plot")
        sphere=px.scatter_3d(sphere_data,x="temperature_C",y="pH",z=view_feature,color="temperature_C",symbol="dataset",hover_name="sample_id",color_continuous_scale="Turbo",opacity=.9)
        sphere.update_traces(marker=dict(size=9,symbol="circle",line=dict(width=.8,color="white")))
        sphere.update_layout(height=630,margin=dict(l=0,r=0,t=45,b=0),title=f"{view_feature.replace('_',' ')} across pH and temperature",scene=dict(xaxis_title="Temperature (°C)",yaxis_title="pH",zaxis_title=view_feature.replace('_',' ')))
        st.plotly_chart(sphere,use_container_width=True)

        st.markdown("#### Temperature sensitivity with regression lines")
        fig,ax=plt.subplots(figsize=(10,6)); slope_rows=[]; colours=plt.cm.tab10(np.linspace(0,.8,features.pH.nunique()))
        for colour,(ph_value,group) in zip(colours,features.groupby("pH")):
            mean_group=group.groupby("temperature_C")[view_feature].mean().replace([np.inf,-np.inf],np.nan).dropna().sort_index()
            if len(mean_group)<2: continue
            x=mean_group.index.to_numpy(float); y=mean_group.to_numpy(float); slope,intercept=np.polyfit(x,y,1); fitted=slope*x+intercept
            ss_res=np.sum((y-fitted)**2); ss_tot=np.sum((y-y.mean())**2); r_squared=1-ss_res/ss_tot if ss_tot else np.nan
            ax.scatter(x,y,s=70,color=colour,label=f"pH {ph_value:g}"); ax.plot(x,fitted,color=colour,lw=2)
            slope_rows.append({"pH":ph_value,"Slope per °C":slope,"Intercept":intercept,"R²":r_squared})
        ax.set(xlabel="Temperature (°C)",ylabel=view_feature.replace("_"," "),title=f"Temperature Sensitivity of {view_feature.replace('_',' ').title()}")
        ax.grid(alpha=.2); ax.legend(title="pH",frameon=False,ncol=2); fig.tight_layout(); st.pyplot(fig,use_container_width=True); plt.close(fig)
        st.caption("Slope and R² are placed in the table below to prevent text overlap on the graph.")
        st.dataframe(pd.DataFrame(slope_rows).style.format({"Slope per °C":"{:.6g}","Intercept":"{:.6g}","R²":"{:.4f}"}),use_container_width=True,hide_index=True)
...
with selection_tab:
    ...
    if "ranking" in st.session_state:
        ...
        fig, ax = plt.subplots(figsize=(9, max(4, top_n*.32))); plot = ranking.head(top_n).sort_values("Combined_Score"); ax.barh(plot.Feature, plot.Combined_Score); ax.set_xlabel("Combined feature-selection score"); st.pyplot(fig); plt.close(fig)
        corr = st.session_state.clean_train[st.session_state.selected].corr()
        fig, ax = plt.subplots(figsize=(9,7)); sns.heatmap(corr, cmap="coolwarm", center=0, ax=ax); ax.set_title("Selected-feature correlation"); st.pyplot(fig); plt.close(fig)
        if len(st.session_state.selected) >= 2:
            ...
            ax.set_xlabel("PC1"); ax.set_ylabel("PC2"); fig.colorbar(sc, label="pH"); ax.set_title("PCA of selected features"); st.pyplot(fig); plt.close(fig)
    ...
    if "selected" not in st.session_state: st.info("Calculate a feature ranking and select features first.")
        ...
        if st.button("Train and evaluate models", type="primary", disabled=not selected_models):
            ...
            if val.empty or test.empty: st.error("Validation or test data is empty. Each pH-temperature group needs at least 3 curves.")
                ...
                st.session_state.pop("models",None); gc.collect()
                models=model_set(len(selected),len(train)); rows=[]; predictions={}; best_fitted=None; best_name=None; best_rmse=np.inf
                ...
                for idx,name in enumerate(selected_models):
                    try:
                        ...
                        validation_rmse=np.sqrt(mean_squared_error(yv,pv)); predictions[name]={"validation":pv,"test":pt}
                        rows.append({"Model":name,"Validation_R2":r2_score(yv,pv),"Validation_RMSE":validation_rmse,"Validation_MAE":mean_absolute_error(yv,pv),
                                     "Test_R2":r2_score(yt,pt),"Test_RMSE":np.sqrt(mean_squared_error(yt,pt)),"Test_MAE":mean_absolute_error(yt,pt)})
                        if validation_rmse<best_rmse: best_fitted,best_name,best_rmse=model,name,validation_rmse
                        else: del model
                ...
                st.session_state.update(results=results,models={best_name:best_fitted},best_model_name=best_name,predictions=predictions,test_frame=test)
...
with comparison_tab:
    if "results" not in st.session_state: st.info("Train the models first.")
        ...
        classes=np.sort(st.session_state.features.pH.unique()); metric_rows=[]
        for model_name in results.Model: metric_rows.append({"Model":model_name,**classification_metrics(test.pH.to_numpy(),predictions[model_name]["test"],classes)})
        class_table=pd.DataFrame(metric_rows)
        st.markdown("### TP, TN, FP, FN and classification metrics")
        st.caption("TP/TN/FP/FN are summed across one-vs-rest pH classes; precision, recall, specificity and F1 are macro averages.")
        st.dataframe(class_table.style.format({c:"{:.4f}" for c in class_table.columns if c not in ["Model","TP","TN","FP","FN"]}),use_container_width=True)
        metric = st.selectbox("Comparison metric", ["Validation_RMSE","Validation_MAE","Validation_R2","Test_RMSE","Test_MAE","Test_R2"])
        fig, ax = plt.subplots(figsize=(9,5)); ordered=results.sort_values(metric,ascending="R2" not in metric); sns.barplot(data=ordered,x=metric,y="Model",ax=ax); st.pyplot(fig); plt.close(fig)
        name = st.selectbox("Detailed model plots", results.Model)
        actual=test.pH.to_numpy(); predicted=predictions[name]["test"]
        c1,c2=st.columns(2)
        with c1:
            fig,ax=plt.subplots(figsize=(5,5)); ax.scatter(actual,predicted,c=test.temperature_C,cmap="plasma"); lo=min(actual.min(),predicted.min()); hi=max(actual.max(),predicted.max()); ax.plot([lo,hi],[lo,hi],"k--"); ax.set(xlabel="Actual pH",ylabel="Predicted pH",title=f"{name}: actual vs predicted"); st.pyplot(fig); plt.close(fig)
        with c2:
            fig,ax=plt.subplots(figsize=(5.5,5)); actual_classes=nearest(actual,classes)
            for ph_class in classes:
                binary=(actual_classes==ph_class).astype(int)
                if binary.min()==binary.max(): continue
                fpr,tpr,_=roc_curve(binary,-np.abs(predicted-ph_class)); ax.plot(fpr,tpr,lw=2,label=f"pH {ph_class:g} (AUC={auc(fpr,tpr):.2f})")
            ax.plot([0,1],[0,1],"k--",alpha=.6); ax.set(xlabel="False-positive rate",ylabel="True-positive rate",title=f"{name}: one-vs-rest ROC"); ax.legend(fontsize=8); ax.grid(alpha=.2); fig.tight_layout(); st.pyplot(fig); plt.close(fig)
        ...
        fig,ax=plt.subplots(figsize=(6,5)); sns.heatmap(cm,annot=True,fmt="d",cmap="Blues",xticklabels=classes,yticklabels=classes,ax=ax); ax.set(xlabel="Predicted pH class",ylabel="Actual pH class",title=f"{name}: confusion matrix"); st.pyplot(fig); plt.close(fig)
        ...
        st.download_button("Download Excel results",excel_bytes({"Regression":results,"Classification":class_table,"Test_Predictions":table}),"CV_model_results.xlsx")
        best_saved=st.session_state.best_model_name; buffer=io.BytesIO(); joblib.dump({"model":st.session_state.models[best_saved],"selected_features":st.session_state.selected,"pH_classes":classes},buffer)
        st.download_button(f"Download best trained model: {best_saved}",buffer.getvalue(),f"{best_saved.replace(' ','_')}_model.joblib")
    ...
    if "results" not in st.session_state:
        ...
        with left:
            ...
            if not raw_matches.empty:
                ...
                ax.grid(alpha=.2); fig.tight_layout(); st.pyplot(fig); plt.close(fig)
        with right:
        ...
        with feature_right:
            ...
            ax.set_title("Test-CV feature profile relative to training data"); fig.tight_layout(); st.pyplot(fig); plt.close(fig)

        st.markdown("### 🎬 Video-like machine-learning prediction")
        gif_key=f"{sample['sample_id']}::{prediction_model}"
        if st.button("Generate prediction GIF",use_container_width=True,disabled=raw is None):
            with st.spinner("Animating CV acquisition and evolving prediction..."):
                st.session_state.prediction_gif=make_prediction_gif(raw,predicted_ph,actual_ph); st.session_state.prediction_gif_key=gif_key
        if st.session_state.get("prediction_gif_key")==gif_key:
            st.image(st.session_state.prediction_gif,caption="CV acquisition and evolving pH prediction",use_container_width=True)
            st.download_button("Download prediction GIF",st.session_state.prediction_gif,"pH_prediction.gif",mime="image/gif",use_container_width=True)
        ...
        st.markdown("### Complete report downloads")
        report_key=f"{gif_key}::{location_name}"
        if st.button("Prepare PDF, PowerPoint and PNG package",type="primary",use_container_width=True,disabled=raw is None):
            with st.spinner("Preparing report files..."):
                st.session_state.report_bundle=make_report_bundle(location_name,local_now.strftime("%Y-%m-%d %H:%M:%S %Z"),results,sample,predicted_ph,prediction_model,raw,test_feature_table)
                st.session_state.report_bundle_key=report_key
        if st.session_state.get("report_bundle_key")==report_key:
            pdf,pptx,png_zip=st.session_state.report_bundle; d1,d2,d3=st.columns(3)
            d1.download_button("Download PDF",pdf,"AI_TC_pH_Report.pdf",mime="application/pdf",use_container_width=True)
            d2.download_button("Download PowerPoint",pptx,"AI_TC_pH_Report.pptx",mime="application/vnd.openxmlformats-officedocument.presentationml.presentation",use_container_width=True)
            d3.download_button("Download PNG ZIP",png_zip,"AI_TC_pH_PNG_Figures.zip",mime="application/zip",use_container_width=True)
