...
def read_cv_file(uploaded):
    ...
    for j, col in enumerate(columns):
        ...
        if len(v) < 10:
            continue
        # Limit extremely long curves so one upload cannot exhaust free RAM.
        if len(v) > MAX_CV_POINTS:
            keep = np.linspace(0, len(v)-1, MAX_CV_POINTS, dtype=int)
            v, i = v[keep], i[keep]
...
def rank_features(train_features):
    ...
    # Free-tier setting: enough trees for stable ranking without exhausting RAM.
    forest = RandomForestRegressor(n_estimators=40, max_depth=9, random_state=RANDOM_STATE, n_jobs=1).fit(clean, y)
    pi = permutation_importance(forest, clean, y, n_repeats=2, random_state=RANDOM_STATE, n_jobs=1)
...
def model_set(feature_count, train_count):
    ...
    return {"Linear Regression": scaled(LinearRegression()), "Ridge Regression": scaled(Ridge(alpha=1)),
            ...
            "Random Forest": plain(RandomForestRegressor(n_estimators=40, max_depth=9, random_state=RANDOM_STATE, n_jobs=1)),
            "Extra Trees": plain(ExtraTreesRegressor(n_estimators=40, max_depth=9, random_state=RANDOM_STATE, n_jobs=1)),
            "Gradient Boosting": plain(GradientBoostingRegressor(n_estimators=60, learning_rate=.06, max_depth=2, random_state=RANDOM_STATE)),
            "Histogram Gradient Boosting": plain(HistGradientBoostingRegressor(max_iter=60, learning_rate=.06, random_state=RANDOM_STATE))}
...
with feature_tab:
    ...
    if "features" in st.session_state:
        ...
        fig, ax = plt.subplots(figsize=(8,4)); sns.boxplot(data=features, x="pH", y=view_feature, hue="temperature_C", ax=ax); ax.legend(ncol=4, fontsize=8); st.pyplot(fig); plt.close(fig)
...
with training_tab:
    if "selected" not in st.session_state: st.info("Calculate a feature ranking and select features first.")
        ...
        available_models=list(model_set(len(st.session_state.selected),10))
        safe_default=["Ridge Regression","PLS Regression","SVR","Random Forest","Extra Trees"]
        selected_models = st.multiselect(
            "Models to train (five lightweight models are selected for the free tier)",
            available_models, default=safe_default,
        )
        if st.button("Train and evaluate models", type="primary", disabled=not selected_models):
            # Remove an earlier training run before allocating new model objects.
            st.session_state.pop("models",None); st.session_state.pop("predictions",None)
            st.session_state.pop("results",None); gc.collect()
            ...
            if val.empty or test.empty: st.error("Validation or test data is empty. Each pH-temperature group needs at least 3 curves.")
                ...
                models=model_set(len(selected),len(train)); rows=[]; predictions={}
                best_fitted_model=None; best_fitted_name=None; best_validation_rmse=np.inf
                ...
                for idx,name in enumerate(selected_models):
                    try:
                        ...
                        validation_rmse=np.sqrt(mean_squared_error(yv,pv))
                        predictions[name]={"validation":pv,"test":pt}
                        ...
                        # Retain only the best fitted model; predictions from every
                        # model are small arrays and remain available for comparison.
                        if validation_rmse < best_validation_rmse:
                            best_fitted_model=model; best_fitted_name=name; best_validation_rmse=validation_rmse
                        else:
                            del model
                ...
                st.session_state.update(results=results, models={best_fitted_name:best_fitted_model}, best_model_name=best_fitted_name, predictions=predictions, test_frame=test)
...
with comparison_tab:
    if "results" not in st.session_state: st.info("Train the models first.")
        ...
        with c2:
            fig,ax=plt.subplots(figsize=(5,5)); residual=predicted-actual; ax.scatter(predicted,residual,c=test.temperature_C,cmap="plasma"); ax.axhline(0,color="black",ls="--"); ax.set(xlabel="Predicted pH",ylabel="Residual (predicted − actual)",title=f"{name}: residuals"); st.pyplot(fig); plt.close(fig)
        ...
        buffer=io.BytesIO(); joblib.dump({"model":st.session_state.models[best_name],"selected_features":st.session_state.selected,"pH_classes":classes},buffer)
        st.download_button(f"Download best trained model: {best_name}",buffer.getvalue(),f"{best_name.replace(' ','_')}_model.joblib")
...
st.sidebar.success("Free-tier memory mode is active: lightweight ensembles, single-core training and automatic plot cleanup.")
if st.sidebar.button("🧹 Clear memory and restart", use_container_width=True):
    st.session_state.clear()
    plt.close("all")
    gc.collect()
    st.rerun()
st.sidebar.info("If the app was previously over capacity, reboot it once after uploading this optimized file, then run the stages again.")
