...
def rank_features(train_features):
    ...
    forest = RandomForestRegressor(n_estimators=80, max_depth=12, random_state=RANDOM_STATE, n_jobs=1).fit(clean, y)
    pi = permutation_importance(forest, clean, y, n_repeats=4, random_state=RANDOM_STATE, n_jobs=1)
...
def model_set(feature_count, train_count):
    ...
    return {"Linear Regression": scaled(LinearRegression()), "Ridge Regression": scaled(Ridge(alpha=1)),
            ...
            "Random Forest": plain(RandomForestRegressor(n_estimators=80, max_depth=12, random_state=RANDOM_STATE, n_jobs=1)),
            "Extra Trees": plain(ExtraTreesRegressor(n_estimators=80, max_depth=12, random_state=RANDOM_STATE, n_jobs=1)),
            "Gradient Boosting": plain(GradientBoostingRegressor(n_estimators=120, learning_rate=.05, max_depth=3, random_state=RANDOM_STATE)),
            "Histogram Gradient Boosting": plain(HistGradientBoostingRegressor(max_iter=120, learning_rate=.05, random_state=RANDOM_STATE))}
