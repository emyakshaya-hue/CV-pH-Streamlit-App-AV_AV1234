...
with upload_tab:
    ...
    if "cv_data" in st.session_state:
        ...
        st.markdown("### Mean CV plots at a selected temperature")
        selected_temperature = st.selectbox(
            "Select temperature",
            options=sorted(data["temperature_C"].unique()),
            key="mean_cv_selected_temperature",
            help="The app averages all replicate CV curves separately for pH 3, 4, 5, 6 and 7.",
        )
        ph_targets = [3, 4, 5, 6, 7]
        ph_colours = {3:"#e63946", 4:"#f59e0b", 5:"#10b981", 6:"#168aad", 7:"#7b2cbf"}
        for row_targets in [ph_targets[:3], ph_targets[3:]]:
            plot_columns = st.columns(len(row_targets))
            for plot_column, ph_value in zip(plot_columns, row_targets):
                with plot_column:
                    subset = data[
                    ...
                    if mean_result is None:
                        st.warning(f"No CV data: pH {ph_value} at {selected_temperature:g} °C")
                        continue
                    mean_voltage, mean_current, current_sd = mean_result
                    colour = ph_colours[ph_value]
                    fig, ax = plt.subplots(figsize=(5.0, 3.8))
                    ax.plot(mean_voltage, mean_current, color=colour, linewidth=2.4, label="Mean CV")
                    ax.fill_between(
                        mean_voltage,
                        mean_current-current_sd,
                        mean_current+current_sd,
                        color=colour,
                        alpha=.20,
                        label="±1 SD",
                    ...
                    ax.set_title(
                        f"pH {ph_value} at {selected_temperature:g} °C\n{len(subset)} replicate CV curves",
                        fontweight="bold",
                    )
                    ax.set_xlabel("Potential (V)")
                    ax.set_ylabel("Mean current (nA)")
                    ax.grid(alpha=.20)
